...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        return 1
    return sum([num_trees(i) * num_trees(i - 1) for i in range(1, n)])
...
class VendingMachine:
    ...
    def add_funds(self,pay_money):
        if self.inventory <= 0:
            return f'Nothing left to vend. Please restock. Here is your ${pay_money}.'
        else:
            self.balance += pay_money
...
def reverse_other(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def reverse_other_inner(t,i):
        if i % 2 == 1:
            for j in range(len(t.branches) // 2):
                t.branches[j].label,t.branches[len(t.branches) - 1 - j].label = t.branches[len(t.branches) - 1 - j].label,t.branches[j].label
        for b in t.branches:
            reverse_other_inner(b,i + 1)
    reverse_other_inner(t,1)
